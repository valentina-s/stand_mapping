...
def get_covers_from_extent(bbox, epsg, api_key,
                          weights=[0.25, 0.25, 0.25, 0.25]):
    """
    Retrieve land cover classification image from the Microsoft AI for Earth,
    v2 API within a user-defined bounding box.
    ...
    print('Retrieving image from AI for Earth Land Cover API', end='...')
    r = requests.post(BASE_URL, json=extent, headers=api_header).json()
    output_hard = imread(io.BytesIO(base64.b64decode(r['output_hard'])))
    output_soft = imread(io.BytesIO(base64.b64decode(r['output_soft'])))
    print('Done.')
    
    return output_hard, output_soft


def get_naip_from_extent(bbox, epsg, api_key,
                          weights=[0.25, 0.25, 0.25, 0.25]):
    """
    Retrieve 3-band NAIP image from the Microsoft AI for Earth,
    v2 API within a user-defined bounding box.
